...
pub const V2_AUTHORITY_RESPONSE_SCHEMA_VERSION: u32 = 14;
...
pub struct V2AuthorityResponseV1 {
    ...
    #[serde(default)]
    pub resource_invocations: Vec<V2AuthorityResourceInvocationResolutionV1>,
    #[serde(default)]
...
#[derive(Debug, Clone, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub struct V2AuthorityResourceInvocationResolutionV1 {
    pub id: String,
    pub identity: V2AuthorityIdentityV1,
    pub resource_identity: V2AuthorityIdentityV1,
    pub module_specifier: String,
    pub export_name: String,
}
...
pub fn validate_v2_authority_response_v1(
    ...
    validate_resource_invocation_family(
        &request.resource_invocations,
        &response.resource_invocations,
    )?;
...
pub fn v2_authoring_resolutions_from_response_v1(
    ...
    Ok(V2AuthoringResolutionsV1 {
        ...
        server_action_invocations: response
            ...
            .collect::<Result<Vec<_>, V2AuthorityResponseErrorV1>>()?,
        resource_invocations: response
            .resource_invocations
            .iter()
            .map(|resolution| {
                Ok(crate::ResolvedResourceInvocationV1 {
                    callee_source: range_from_site_id(
                        &resolution.id,
                        "resource-invocation",
                        &parsed.syntax.source,
                    )?,
                    resource_identity: identity_from_response(&resolution.resource_identity),
                    module_specifier: resolution.module_specifier.clone(),
                    export_name: resolution.export_name.clone(),
                    declaration_modules: resolution.identity.declaration_modules.clone(),
                })
...
fn validate_resource_invocation_family(
    request: &[crate::v2_authority_request::V2AuthorityResourceInvocationSiteV1],
    response: &[V2AuthorityResourceInvocationResolutionV1],
) -> Result<(), V2AuthorityResponseErrorV1> {
    let allowed = request
        .iter()
        .map(|site| (site.id.as_str(), site))
    ...
    for resolution in response {
        ...
        if resolution.module_specifier != site.module_specifier
            ...
            || resolution.identity.name != site.export_name
            || resolution.identity.declaration_modules.is_empty()
            || resolution.resource_identity.name != "resource"
            || resolution.resource_identity.declaration_modules.is_empty()
        {
            return Err(V2AuthorityResponseErrorV1::IncompatibleSite(
                resolution.id.clone(),
            ));
    ...
    if seen.len() != allowed.len() {
        let missing = allowed
            .keys()
            .find(|id| !seen.contains(**id))
            .expect("different lengths imply one missing Resource site");
        return Err(V2AuthorityResponseErrorV1::IncompatibleSite(
            (*missing).to_owned(),
        ));
    }
