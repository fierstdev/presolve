...
pub fn build_semantic_capability_registry() -> SemanticCapabilityRegistry {
    SemanticCapabilityRegistry {
        ...
        capabilities: vec![
            ...
            admitted(
                "external_package_inputs",
                SemanticCapabilityClass::Native,
                "any platform-compatible package import used outside a Presolve semantic position",
                "TypeScript module resolution and the project Vite publication pass",
                "the package manager and Vite must resolve the selected browser export",
                "Presolve records no semantic dependency for an erased or ordinary bundled use",
                "the package owns its own runtime behavior; Presolve owns no resumable state for this class",
                "the package is present only in Vite output and the publication inventory",
                "examples/package-interop plus application_semantic_model::tests::file_route_assembly_admits_only_authority_proven_terminal_package_invocations",
            ),
            admitted(
                "terminal_package_invocation",
                SemanticCapabilityClass::Bounded,
                "decorator-free action(() => { namedPackageImport(); }) with one synchronous zero-argument result-discarded call",
                "TypeScript declaration identity, canonical V2 Action endpoint, package invocation artifact, and Vite registry",
                "one exact named import; the call must be the sole statement and must accept zero arguments",
                "the invocation participates in diagnostics and Action event ownership but declares no reactive dependencies",
                "the Action endpoint is restored through the normal event plan; the package owns no snapshot slot",
                "package-invocations.runtime.json and /presolve.package-invocations.js",
                "examples/package-interop and real-browser click proof",
            ),
            admitted(
            ...
            admitted(
                ...
                SemanticCapabilityClass::Bounded,
                "Math.min(left, right) or Math.max(left, right) in a compiler-derived reactive getter",
            ...
            admitted(
                ...
                SemanticCapabilityClass::Bounded,
                "Math.floor(value), Math.ceil(value), or Math.round(value) in a compiler-derived reactive getter",
            ...
            admitted(
                "resources",
                SemanticCapabilityClass::Bounded,
                "field: Resource<Data, Error> = resource(async (context: ResourceContext) => imported(context))",
                "TypeScript authority, integrity-checked semantic-package endpoint, Resource declaration and activation identity, Vite package bundling, browser runtime artifact, and generated runtime",
                "one exact named-import semantic-package resource export, Resource<Data, Error> field type, canonical ResourceContext parameter, Promise completion, and client/shared execution boundary; use loader(...) on a route for server execution",
                "one compiler-owned cold activation per planned component instance; direct same-owner reactive-getter .data/.error/.state reads and terminal invalidation are admitted, while inputs and retry remain deferred",
                ...
                "authority request/response schema v14, authored semantics schema v9, runtime resource schema v4, runtime-computed schema v13, embedded page artifacts, exact resume slots/codecs, and a content-addressed browser bundle",
                "ergonomic_project::canonical_general_resource_bundles_and_activates_a_browser_endpoint",
            ),
            admitted(
                "route_loaders",
                SemanticCapabilityClass::Bounded,
                "field: Resource<Data, Error> = loader(async (params: RouteParameters, signal: AbortSignal) => imported(params, signal)) on a conventional route page",
                "TypeScript authority, compiler route graph, Resource declaration/activation, binding table, semantic-package contract, RouteLoaderPlanV2, and Node registry",
                "one exact named-import server/shared resource export with route_parameters input, valid cache policy, typed failure, canonical RouteParameters and AbortSignal parameters, and Promise completion",
                "compiler-normalized parameters, exact data/error codecs, Resource bootstrap identity, cache key/partition facts, and compiler-owned reactive dependencies",
                "server executes per request with disconnect/shutdown abort; schema-v4 bootstrap restores one ready/failed generation before dependent browser reads; pending work is never serialized",
                "route-loaders.plan.json schema v2, runtime Resource schema v4, and Node deployment schema v3 with digest-bound loader registry",
                "ergonomic_project::canonical_route_loader_bundles_executes_caches_and_bootstraps_the_browser",
            ),
            admitted(
                "server_actions",
                SemanticCapabilityClass::Bounded,
                "defineForm({ fields, submit: async ({ formData, signal }) => imported(formData, signal) }) on a conventional route page",
                "TypeScript authority, compiler route graph, canonical Form binding, semantic-package contract, and RouteServerActionPlanV1 publication",
                ...
                "the submit body is one exact authority-proven package call; the compiler records the closed server capability without treating arbitrary application code as server executable",
                ...
                "runtime_browser::canonical_form_server_action_executes_through_node_and_browser",
            ),
            admitted(
                "opaque_typescript",
