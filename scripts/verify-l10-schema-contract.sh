...
compatibility_manifest=crates/presolve_compiler/fixtures/tooling-schema/l3-l8-canonical-fixtures-v1.sha256
...
actual_paths="$({
  find crates/presolve_compiler/fixtures/platform -name '*.json' -type f
  find crates/presolve_compiler/fixtures/service -name '*.json' -type f
  find crates/presolve_compiler/fixtures/incremental -name '*.json' -type f
  find crates/presolve_compiler/fixtures/persistent-cache -name '*.json' -type f
  find crates/presolve_compiler/fixtures/workspace -name '*.json' -type f
  find crates/presolve_compiler/fixtures/watch -name '*.json' -type f
  printf '%s\n' crates/presolve_cli/fixtures/configuration/minimum-l3-v1.json
  printf '%s\n' crates/presolve_cli/fixtures/configuration/nonempty-l3-v1.json
} | LC_ALL=C sort)"
...
if rg -n 'use crate::(service|persistent_cache|workspace|watch)' crates/presolve_compiler/src/tooling_schema.rs; then
  echo 'L10 negotiation must remain independent from L3-L8 execution and durable products' >&2
  exit 1
fi
if rg --quiet 'tooling_schema' crates/presolve_compiler/src/{platform,service,persistent_cache,workspace,watch}.rs crates/presolve_cli/src; then
  echo 'L3-L8 products and CLI dispatch must not depend on the L10 registry' >&2
  exit 1
fi
if ! rg --quiet 'fn decode_tooling_(build_trace|compile_cost_report|artifact_graph|query_snapshot)_v1' crates/presolve_compiler/src/tooling_products.rs; then
