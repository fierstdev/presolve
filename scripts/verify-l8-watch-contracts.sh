...
for fixture in crates/ezc_core/fixtures/watch/*.json; do
  test -s "$fixture"
  test "$(tail -c 1 "$fixture")" = "$(printf '\n')"
  rg -q '"schema"' "$fixture"
done
rg -q '"single-immediate-candidate"' crates/ezc_core/fixtures/watch/scenarios-v1.json
rg -q '"l6-restart-hit"' crates/ezc_core/fixtures/watch/scenarios-v1.json
rg -q '"l5-content-edit-reuse"' crates/ezc_core/fixtures/watch/scenarios-v1.json
if rg -n -i 'fswatch|notify::|recommendedwatcher|inotify|kqueue|read_dir|walkdir|glob::|metadata\(' crates/ezc_core/src/watch.rs; then
  echo "L8 watch implementation must not discover filesystem state" >&2
  exit 1
fi
if rg -n 'SOURCE_SENTINEL|watch-authored-source' crates/ezc_core/fixtures/watch docs/watch-mode-contract.md; then
