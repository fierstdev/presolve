...
* Slice: N9-D - Opaque Terminal Artifact Projection
...
* Slice: N9-F - Opaque Terminal Generated Runtime
* Result: generated runtime parses and validates only schema-v1 opaque artifacts, associates a terminal with the event's compiler-owned method ID, dynamically imports only the exact emitted module location, verifies the declared export is callable, and invokes it with no arguments. Failures become `PSR_OPAQUE_TERMINAL_FAILURE` diagnostics without a State/Form/Context write path. Opaque terminals force resume restoration to `OpaqueTerminalColdFallback` before any restore execution.
...
* Next: N6-C14 now has an authored read design: implement direct same-owner Computed `.data`/`.error`/`.state` projections as exact Resource IR reads, per-instance invalidations, terminal browser updates, and fail-closed resume rejection. Do not introduce a framework Resource store or direct render read.
...
* Verification: `./scripts/verify-m4-publication-audit.sh`, the Phase M proposal, public identity, repository-layout, formatting, and diff checks pass. The browser portion requires approved loopback-server permission in this environment.
* Next: M5 may select one exact existing reactive declaration family; do not create new syntax or a framework runtime.
...
Phase M is frozen through M10, Phase N through N10, and Phase O is archived
complete through O4. Phase P is frozen through P5. It supplies one
...
`@presolve/application` command projection. Routing/server and deployment are
future independent intake topics, not unfinished Phase O work. The next work
requires a new separately authorized product contract.
