...
| Forms | `docs/forms-contract.md`; `crates/presolve_compiler/src/form_submission_host.rs` | `examples/forms/src/Forms.tsx` | explicit `presolve check` proof | browser submit/network behavior |
| Explicit workspace | `crates/presolve_compiler/fixtures/workspace/{chain-v1,cycle-v1}.json`; `crates/presolve_cli/tests/l9_cli_commands.rs` | `examples/explicit-workspace/src/main.ts` | explicit `presolve workspace` proof | manifests/dependency discovery |
| Production/resume | `fixtures/0047-computed-diamond`; `docs/production-optimization-contract.md`; `crates/presolve_cli/tests/runtime_browser.rs` | `examples/production-resume/src/ComputedDiamond.tsx` | explicit production build plus the frozen browser probe | benchmarks, deployment, author-time edits |
