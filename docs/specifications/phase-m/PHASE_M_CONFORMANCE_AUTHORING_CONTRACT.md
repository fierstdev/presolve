...
| Forms | `@form() profile!: Form;`, `@field("profile") name = "";`, `@action() @submit("profile") save() {}`, and `<form form={this.profile}>` | `field` and `submit` use compiler-resolved local Form designators because instance expressions are not valid TypeScript decorator arguments. The host keeps its instance expression because it executes in `render`. Framework aliases, generated hosts, async/network submission, and DOM-derived ownership are unavailable. |
