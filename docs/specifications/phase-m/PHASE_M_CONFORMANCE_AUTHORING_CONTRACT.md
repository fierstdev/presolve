...
| Forms | `@form() profile!: Form;`, `@field("profile") name = "";` or `@field("profile", "identity.name") name = "";`, `@action() @submit("profile") save() {}`, and `<form form={this.profile}>` | `field` and `submit` use compiler-resolved local Form designators because instance expressions are not valid TypeScript decorator arguments. The optional static path is a compiler-validated serialization projection only; it does not change the leaf Field identity or create an object API. The host keeps its instance expression because it executes in `render`. Framework aliases, generated hosts, async/network submission, and DOM-derived ownership are unavailable. |
