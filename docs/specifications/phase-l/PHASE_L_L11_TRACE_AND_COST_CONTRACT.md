...
The report is emitted only for an existing successful Phase K production report pair from the same `build_id` and policy. A producer rejects mismatched, missing, malformed, noncanonical, or incompatible report inputs; it does not estimate or reconstruct them from generated files.
