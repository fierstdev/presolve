...
| --- | --- | --- | --- | --- |
| Compiler/platform | `crates/presolve_compiler` tests; `verify-l3` through `verify-l8` | products and lifecycle | `cargo test -p presolve-compiler --lib` | canonical bytes |
...
Compiler fixtures remain in their owning Rust crate and CLI fixtures remain in `crates/presolve_cli`. Package fixtures may consume but never copy, rewrite, or decode them. SHA fixtures are canonical output commitments, not performance baselines. Browser probes require Chrome and are a separate runtime lane.
