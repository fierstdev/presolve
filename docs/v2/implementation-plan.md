...
| 24 | Effects | `effect_field_lowering`; `effect`, `effect_*`, `runtime_effect_*` | Authority-backed decorator-free Effect fields now enter the canonical component/effect model without synthetic methods. Ordered main/cleanup source bodies lower to separate IR and artifact programs; runtime schema v5 carries cleanup, source declaration order, V2-only post-resume eligibility, and serialized instance ownership, preserving the frozen legacy-decorator resume boundary. The browser executes V2 records in their exact instance context, keys cleanup storage by instance identity, and disposes application/page cleanups child-before-parent in reverse field order. Cleanup-bearing fields remain fail-closed pending the authored structural-template activation and removal contract; static instance records must not be mistaken for dynamic structural occurrences. |
