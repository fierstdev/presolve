...
| 22 | Actions | `presolve_parser::{model,oxc_adapter}`; `presolve_compiler::{action_field_lowering,action_authority}`; later TypeScript adapter | Implemented authored schema v1 action-fact authority with capture and server-import rejection; runtime activation remains a later adopter. Canonical V2 `action(handler)` fields now join direct instance-field calls of proven components to resolved action identity without decorator or callee-spelling recognition. The parser now retains strict inline-handler body facts—exact spans, async/expression form, supported state updates, and unsupported statement spans—without classifying the call. Action projection and cold/resume runtime adoption remain pending. |
