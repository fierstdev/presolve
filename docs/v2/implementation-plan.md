...
| 3 | Semantic authority adapter | `packages/typescript-authority`; TypeScript native async API; compatibility corpus | Implemented schema v1 semantic query boundary for symbols, aliases, types, contextual types, signatures, assignability, diagnostics, resolved modules, and ordered component base-symbol chains. Framework meaning remains registry-owned. |
...
| 7 | Canonical authored semantic model | `presolve_compiler::authored_semantics`, `v2_authoring_lowering`; `presolve_parser::ParsedFile`; `packages/typescript-authority` | Implemented schema v1 boundary for syntax-selected, TypeScript-resolved intrinsic facts and TSX bindings/events; deterministic deduplicated models with source provenance. The decorator-free component, State, and Action lowerings now compose through this one model rather than publishing disconnected feature products. |
| 8 | Legacy decorator lowering | `presolve_compiler::legacy_decorator_lowering`; `presolve_parser::ParsedFile`; `packages/typescript-authority` | Implemented alpha-only adapter-fed lowering from class/property/method/parameter decorator spans to canonical candidates; arbitrary decorator spelling remains unrecognized.  It is not the V2 source surface; see `authoring-syntax-cutover-contract.md`. |
...
| PR | Scope | Repository trace | Products and proof |
| ---: | --- | --- | --- |
| 19 | Structural components and props | `presolve_compiler::structural_component`, `component_inheritance_lowering`, `v2_authoring_lowering`; `component_*`; `composition_typing`; TypeScript authority | Implemented schema v1 TypeScript-authoritative inheritance and props projection; unresolved route generics fail early and `children` is never implicitly injected. Canonical component lowering now joins source-AST heritage clauses to exact TypeScript proof, including aliases and indirect subclasses. `presolve check` and `build` invoke the installed bridge before legacy assembly for decorator-free candidates, with component proof preceding State/Action selection. The canonical ASM adapter now publishes authority-backed decorator-free component routes without a decorator fallback. Canonical State and the closed synchronous Action subset now have cold/resume browser evidence; broader Action forms remain before PR 19 is beta-complete. |
| 20 | Slots | `presolve_compiler::slot_projection`; `slot`; `slot_binding`; `slot_content`; `slot_field_lowering` | Implemented schema v1 deterministic ownership/composition projection over exact bindings. Authority-proven V2 `slot()` fields now lower into the canonical default/named Slot model without decorators. Component artifact schema v20 partitions exact caller-owned target/text-binding/event membership per rendered conditional or keyed outlet. A decorator-free browser fixture proves transactional reveal/hide, keyed retention/reorder/trim, shared caller State/action/binding/event behavior, cleanup, and rejection of fabricated binding, ownership, and marker evidence. Projected component invocation cloning and Slot capture/resume remain explicitly fail-closed. |
| 21 | State | `presolve_compiler::state_initializer_lowering`; `state_projection`; `state_instance_storage`; existing lowering | Implemented schema v1 instance-qualified State inspection with closed-codec resume admission; update behavior remains owned by existing lowering.  Canonical V2 State recognition now joins direct instance-field initializer calls of proven components to resolved `state` identity without callee-spelling recognition. |
| 22 | Actions | `presolve_parser::{model,oxc_adapter}`; `presolve_compiler::{action_field_lowering,action_authority,component_graph,effect}`; later TypeScript adapter | Implemented authored schema v1 action-fact authority with capture and server-import rejection. Canonical V2 `action(handler)` fields join direct instance-field calls of proven components to resolved action identity without decorator or callee-spelling recognition. The parser retains strict inline-handler body facts—exact spans, typed ordered parameters, serializable local declarations, async/expression form, supported state updates, and unsupported statement spans—without classifying the call. The action-endpoint identity contract authorizes the no-synthetic-method migration now used by the runtime projection. Follow-on contracts admit typed primitive parameters assigned directly to same-typed canonical State through exact static event arguments, and serializable local literals assigned directly to same-typed canonical State; parameters project to the existing `assign_parameter` ordinal while locals project to an ordinary literal `assign`. Cold-browser evidence covers both forms. Async handlers, captures, server imports, event-object forwarding, dynamic arguments, and other handler statements remain explicitly rejected pending their own authored semantics. |
| 23 | Computed getters | `presolve_parser::oxc_adapter`; `presolve_compiler::{computed_getter_lowering,authored_semantics,component_graph,computed_projection}` | Implemented schema-v3 derived-candidate amendment for decorator-free getters: parser-selected, synchronous, non-static, call-free expressions with canonical State and computed-to-computed dependency evidence enter the existing computed graph/runtime path without an intrinsic identity or decorator. The analysis admits only acyclic chains that reach canonical State. Installed-authority CLI and cold/resume browser fixtures prove the subset. Calls and unknown-call coverage remain explicitly outside the current candidate contract. |
| 24 | Effects | `effect_field_lowering`; `effect`, `effect_*`, `runtime_effect_*` | Authority-backed decorator-free Effect fields now enter the canonical component/effect model without synthetic methods. Ordered main/cleanup source bodies lower to separate IR and artifact programs; runtime schema v7 carries cleanup, source declaration order, V2-only post-resume eligibility, static-instance ownership, and exact structural effect templates whose live IDs derive solely from opaque occurrence identity. Component runtime schema v20 gives each structural program its compiler-derived component/node renderer address, exact semantic host and invocation template entities, ordered occurrence templates, compiler-rendered component template HTML with an opaque occurrence placeholder, occurrence State/Computed slot templates, their closed resume codecs and parent-template identity, exact ordinary target/binding/event membership, scoped conditional fragments with exact per-branch invocation membership, keyed item fragments with exact per-item invocation membership, nested occurrence template membership, and compiler-selected Slot-binding plus caller-owned target/binding/event membership for every projected host fragment. Resume manifest v7/snapshot v2 restores compiler-qualified conditional, keyed, and nested occurrences without rendering or DOM discovery; it validates identity, parent topology, State codecs, and exact target-anchor membership before it registers bindings/events and eligible Effects. The decorator-free TypeScript-authority browser fixture proves cold materialization followed by structural resume with live State/action/binding behavior, no DOM reconstruction, and cold fallback for malformed identity, State, and anchor evidence. Structural Effects activate parent-first, clean up child-first before removal, and retain identity through keyed reorder. The older decorator fixture remains compatibility-only. Authority-backed Slot-projected structural hosts now have dedicated cold-browser acceptance for exact ordinary projection membership, lifecycle, and corruption rejection; projected component invocation cloning and Slot-projected resume remain fail-closed follow-on contracts. |
...
| 26 | Context consumption/resume | `consumer`, `context_resolution`, `context_resume`, `runtime_context_artifact` | Implemented canonical provider/default/unresolved consumer resolution and fixed resume-slot projection without runtime lookup or reselection. Real-browser probes prove cold source evaluation and updates, nested exact resume bindings, and fail-closed malformed-binding fallback without reselection. |
...
| 42 | V2 scaffold/examples | `create-presolve`, framework package, Vite package | Implemented conventional V2 project layout, public-environment example, and scaffold package/ergonomic build proof; generated routes and public API examples now use the decorator-free V2 component form.  Vite command configuration remains an explicit later adapter product. |
...
| --- | --- | --- | --- |
| AP1 | Conventional project layout and environment | `create-presolve`, `environment_input`, `environment_ownership`, ergonomic fixtures | Implemented named-file schema-v1 environment admission: `PRESOLVE_PUBLIC_*` values are browser eligible and server values retain names without published values. The V2 authority bridge schema v4 forwards parser-generic static member-call candidates and admits only TypeScript-proven canonical `environment.public` uses, including ordinary modules with no Component declaration. Environment-read lowering schema v1 joins that evidence with the named manifest, records only literal public values with exact provenance, and emits stable fail-closed diagnostics for dynamic, server-owned, unprefixed, undeclared, and missing-manifest reads. Ergonomic `check` and `build` now require caller-selected manifest JSON when proven reads exist, and publish the all-or-nothing browser artifact in the file-route inventory; existing ownership analysis consumes the records as browser/application-lifetime nodes. `@presolve/vite` now digest-verifies that artifact and exposes only its frozen public map through a versioned virtual module. Compiler lowering from authored intrinsic calls to that runtime module remains pending; Vite must not translate source calls or add values itself. |
| AP2 | Routes, layouts, loaders, metadata, and server actions | `route_graph`, `layout_composition`, `route_loader`, `route_server_action`, publication | Implemented routes/layouts/loaders/server-action handoffs and strict route-metadata sidecars joined through compiler-selected source paths into the atomic publication manifest. |
