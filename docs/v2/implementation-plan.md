...
| 24 | Effects | `effect_field_lowering`; `effect`, `effect_*`, `runtime_effect_*` | Authority-backed decorator-free Effect fields now enter the canonical component/effect model without synthetic methods. Ordered main/cleanup source bodies lower to separate IR and artifact programs; runtime schema v6 carries cleanup, source declaration order, V2-only post-resume eligibility, serialized static-instance ownership, and inactive structural templates, preserving the frozen legacy-decorator resume boundary. Component runtime schema v14 gives each structural program its compiler-derived component/node renderer address, exact semantic host and invocation template entities, ordered occurrence templates, compiler-rendered component template HTML with an opaque occurrence placeholder, occurrence State/Computed slot templates, exact inactive ordinary target/binding/event membership, scoped conditional fragments, and compiler-rendered keyed item fragments that retain the existing item/key/index tokens plus exact structural invocation anchors. Compiler and runtime validators reject fabricated membership, scope, or host kind; Slot-projected host fragments remain unavailable until their full compiler scope is modeled. A decorator-free TypeScript-authority browser fixture now verifies the same structural artifact/host boundary; the older decorator fixture remains compatibility-only. The browser executes V2 static records in their exact instance context, keys cleanup storage by instance identity, and disposes application/page cleanups child-before-parent in reverse field order. The structural component materialization contract fixes the required compiler renderer program and opaque dynamic-occurrence boundary before activation. Cleanup-bearing fields remain fail-closed pending materialization through those exact structural addresses, dynamic occurrence identity, and structural removal disposal; static records must not be mistaken for dynamic structural occurrences. |
