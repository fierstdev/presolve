...
| 24 | Effects | `effect_field_lowering`; `effect`, `effect_*`, `runtime_effect_*` | Authority-backed decorator-free Effect fields now enter the canonical component/effect model without synthetic methods. Ordered main/cleanup source bodies lower to separate IR and artifact programs; runtime schema v7 carries cleanup, source declaration order, V2-only post-resume eligibility, static-instance ownership, and exact structural effect templates whose live IDs derive solely from opaque occurrence identity. Component runtime schema v17 gives each structural program its compiler-derived component/node renderer address, exact semantic host and invocation template entities, ordered occurrence templates, compiler-rendered component template HTML with an opaque occurrence placeholder, occurrence State/Computed slot templates, exact ordinary target/binding/event membership, scoped conditional fragments with exact per-branch invocation membership, keyed item fragments with exact per-item invocation membership, and nested occurrence template membership. Compiler and runtime validators reject fabricated membership, scope, host kind, or a missing/duplicate/substituted marker. The decorator-free TypeScript-authority browser fixture proves conditional, keyed, and nested structural Leaves materialize with live State/action/binding/Effect behavior; structural Effects activate parent-first, clean up child-first before removal, retain identity through keyed reorder, and reject malformed artifact ownership. The older decorator fixture remains compatibility-only. Slot-projected and structural-resume lifecycles remain fail-closed pending their own activation paths. |
