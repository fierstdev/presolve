...
| AP2 | Routes, layouts, loaders, metadata, and server actions | `route_graph`, `layout_composition`, `route_loader`, `route_server_action`, publication | Routes/layouts/loaders/server-action handoffs are implemented. Route metadata now has an authored sidecar/publication contract; discovery and artifact emission are the next implementation slice. |
