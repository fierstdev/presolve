...
| PR | Scope | Repository trace | Products and proof |
| ---: | --- | --- | --- |
| 19 | Structural components and props | `presolve_compiler::structural_component`, `component_inheritance_lowering`, `v2_authoring_lowering`; `component_*`; `composition_typing`; TypeScript authority | Implemented schema v1 TypeScript-authoritative inheritance and props projection; unresolved route generics fail early and `children` is never implicitly injected. Canonical component lowering now joins source-AST heritage clauses to exact TypeScript proof, including aliases and indirect subclasses. Build adoption is governed by `authoring-build-adoption-contract.md`: a direct fresh-scaffold probe currently fails before the authority bridge is wired, so PR 19 is not beta-complete. |
