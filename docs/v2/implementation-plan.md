...
| 24 | Effects | `effect_field_lowering`; `effect`, `effect_*`, `runtime_effect_*` | Authority-backed decorator-free Effect fields now enter the canonical component/effect model without synthetic methods. Ordered main/cleanup source bodies lower to separate IR and artifact programs; runtime schema v4 carries cleanup, source declaration order, and V2-only post-resume eligibility, preserving the frozen legacy-decorator resume boundary. The new instance-effect registry joins valid V2 fields to planned nested/repeated component instances with typed IDs, parent/depth, and field order. Cleanup-bearing fields remain fail-closed pending serialization and instance-context execution/disposal. |
