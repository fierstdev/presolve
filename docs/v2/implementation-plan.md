...
| 24 | Effects | `effect_field_lowering`; `effect`, `effect_*`, `runtime_effect_*` | Authority-backed decorator-free Effect fields now enter the canonical component/effect model without synthetic methods. Ordered main/cleanup source bodies lower to separate IR and artifact programs; runtime schema v6 carries cleanup, source declaration order, V2-only post-resume eligibility, serialized static-instance ownership, and inactive structural templates, preserving the frozen legacy-decorator resume boundary. Component runtime schema v15 gives each structural program its compiler-derived component/node renderer address, exact semantic host and invocation template entities, ordered occurrence templates, compiler-rendered component template HTML with an opaque occurrence placeholder, occurrence State/Computed slot templates, exact inactive ordinary target/binding/event membership, scoped conditional fragments with exact per-branch invocation membership, and compiler-rendered keyed item fragments that retain the existing item/key/index tokens plus exact structural invocation anchors. Compiler and runtime validators reject fabricated membership, scope, host kind, or a missing/duplicate/substituted static-conditional marker. The decorator-free TypeScript-authority browser fixture now proves the admitted static-conditional slice: a materialized Leaf has live State/action/binding behavior and its record is removed on branch replacement; the older decorator fixture remains compatibility-only. Keyed, nested, Slot-projected, Effect, cleanup, and resume structural lifecycles remain fail-closed pending their own activation paths. |
