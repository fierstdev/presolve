...
| --- | --- | --- | --- |
| AP1 | Conventional project layout and environment | `create-presolve`, `environment_input`, `environment_ownership`, ergonomic fixtures | Implemented named-file schema-v1 environment admission: `PRESOLVE_PUBLIC_*` values are browser eligible and server values retain names without published values. The V2 authority bridge schema v3 forwards parser-generic static member-call candidates and admits only TypeScript-proven canonical `environment.public` uses. Environment-read lowering schema v1 joins that evidence with the named manifest, records only literal public values with exact provenance, and emits stable fail-closed diagnostics for dynamic, server-owned, unprefixed, undeclared, and missing-manifest reads. The resulting browser-environment artifact is all-or-nothing and contains only source-proven public values, while existing ownership analysis consumes those records as browser/application-lifetime nodes. Project-level manifest selection and application-publication inventory integration remain next. |
