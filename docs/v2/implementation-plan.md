...
| 24 | Effects | `effect_field_lowering`; `effect`, `effect_*`, `runtime_effect_*` | Authority-backed decorator-free Effect fields now enter the canonical component/effect model without synthetic methods. Ordered main/cleanup source bodies lower to separate IR and artifact programs; runtime schema v6 carries cleanup, source declaration order, V2-only post-resume eligibility, serialized static-instance ownership, and inactive structural templates, preserving the frozen legacy-decorator resume boundary. Component runtime schema v5 gives each structural program its compiler-derived component/node renderer address, validated against the template manifest before use. The browser executes V2 static records in their exact instance context, keys cleanup storage by instance identity, and disposes application/page cleanups child-before-parent in reverse field order. Cleanup-bearing fields remain fail-closed pending activation through those exact structural addresses, dynamic occurrence identity, and structural removal disposal; static records must not be mistaken for dynamic structural occurrences. |
