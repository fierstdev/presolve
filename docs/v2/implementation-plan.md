...
| 23 | Computed getters | `presolve_parser::oxc_adapter`; `presolve_compiler::{computed_getter_lowering,authored_semantics,component_graph,computed_projection}` | Implemented schema-v2 derived-candidate amendment for decorator-free getters: parser-selected, synchronous, non-static, call-free expressions with direct canonical State reads gain explicit dependency evidence and enter the existing computed graph/runtime path without an intrinsic identity or decorator. Installed-authority CLI and cold/resume browser fixtures prove the initial subset. Calls, transitive computed dependencies, conditional coverage, and cycles remain explicitly outside the current candidate contract. |
