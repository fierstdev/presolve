...
| 23 | Computed getters | `presolve_compiler::computed_projection`; existing computed products | Implemented schema v1 inspection projection over canonical dependencies, caches, and dirty flags. V2 source adoption is governed by `computed-source-contract.md`: no `computed()` intrinsic exists, so a getter requires reactive-read, purity, and cycle proof plus a versioned non-intrinsic canonical-candidate amendment before runtime adoption. |
