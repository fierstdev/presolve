...
| 22 | Actions | `presolve_parser::{model,oxc_adapter}`; `presolve_compiler::{action_field_lowering,action_authority,component_graph,effect}`; later TypeScript adapter | Implemented authored schema v1 action-fact authority with capture and server-import rejection. Canonical V2 `action(handler)` fields join direct instance-field calls of proven components to resolved action identity without decorator or callee-spelling recognition. The parser retains strict inline-handler body facts—exact spans, async/expression form, supported state updates, and unsupported statement spans—without classifying the call. The action-endpoint identity contract authorizes the no-synthetic-method migration now used by the runtime projection: the closed synchronous `this.<canonical-state>` update subset has compiler, CLI, and cold/resume browser evidence. Async handlers, captures, server imports, parameters, and other handler statements remain explicitly rejected pending their own authored semantics. |
